...
import { Bot, Activity, Clock, Zap, TrendingUp, DollarSign, TrendingDown, AlertTriangle, LineChart, Info, CheckSquare, XSquare, Landmark, Scale } from 'lucide-react'; // Added Gov icons
import { Skeleton } from '@/components/ui/skeleton';
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { ChartContainer, ChartTooltip, ChartTooltipContent } from "@/components/ui/chart";
import { Area, AreaChart, CartesianGrid, XAxis, YAxis } from "recharts";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { getHistoricalData, getGovBondYields, GovBondYield } from '@/services/broker-api'; // Use relevant APIs
...
interface AgentAction {
  id: string;
  timestamp: Date;
  type: 'BUY' | 'SELL' | 'REBALANCE' | 'INFO' | 'ERROR' | 'ENABLE' | 'DISABLE' | 'MODE_CHANGE' | 'STRATEGY_UPDATE'; // Added strategy update
  details: string; // More descriptive text
  amount?: number; // Can be face value for bonds
  ticker?: string;
  priceOrYield?: number; // Price or yield at execution
...
const fetchAgentStatus = async (): Promise<AgentStatus> => {
    ...
    */
    ...
    console.warn("fetchAgentStatus using MOCK data. Requires backend.");
    await new Promise(res => setTimeout(res, 300)); // Simulate delay
    // Simulate a possible state
    const isEnabled = Math.random() > 0.3; // 70% chance enabled
    const mode = isEnabled ? (Math.random() > 0.5 ? 'full' : 'strategy') : 'unknown';
    return {
        isEnabled: isEnabled,
        mode: mode,
        allocatedAmount: mode === 'full' ? 500 : 50, // Example amounts in Millions
        activeStrategies: mode === 'strategy' ? ['yield_curve_positioning', 'inflation_protection'] : [], // Example strategies
    };
...
const fetchAgentActions = async (limit: number = 50): Promise<AgentAction[]> => {
    ...
    */
    ...
    // Placeholder/Mock Implementation
    console.warn("fetchAgentActions using MOCK data. Requires backend.");
    ...
    for (let i = 0; i < limit; i++) {
        ...
        const timestamp = new Date(now.getTime() - i * 60 * 60 * 1000 * Math.random() * 5); // Actions in the last ~10 days
        ...
        switch (type) {
            ...
            case 'SELL':
                ...
                amount = Math.floor(Math.random() * 5 + 1) * 1000000; // $1M - $5M face value
                ...
                break;
            case 'REBALANCE':
                details = `Portfolio rebalance triggered due to duration drift.`;
                strategyContext = 'duration_management';
                break;
            case 'INFO':
                details = `Monitoring market volatility for ${tickers[Math.floor(Math.random() * tickers.length)]}.`;
                break;
            case 'ERROR':
                details = `Failed to execute trade for ${tickers[Math.floor(Math.random() * tickers.length)]}: Broker connection timeout.`;
                break;
            case 'ENABLE': details = `Agent enabled in ${Math.random() > 0.5 ? 'full' : 'strategy'} mode.`; break;
            case 'DISABLE': details = `Agent disabled by user.`; break;
            case 'MODE_CHANGE': details = `Agent mode changed to ${Math.random() > 0.5 ? 'full' : 'strategy'} automation.`; break;
            case 'STRATEGY_UPDATE': details = `Applied new strategies: [${strategies[0]}, ${strategies[1]}]`; break;
        }

        actions.push({
            id: `act_${timestamp.getTime()}_${i}`,
            timestamp: timestamp,
            type: type,
            details: details,
            amount: amount,
            ticker: ticker,
            priceOrYield: priceOrYield,
...
// Chart configuration
const chartConfig = {
  value: { label: "Yield", color: "hsl(var(--primary))" }, // Label adjusted for yields
  ...
  useEffect(() => {
   ...
   useEffect(() => {
    const fetchYieldData = async () => {
      ...
      setYieldCurveData([]);
      try {
        const yields = await getGovBondYields();
         if (!Array.isArray(yields) || yields.length === 0) {
             throw new Error("No yield curve data returned.");
         }
        // Map to format expected by recharts { name: maturity, value: yield }
        const formattedYields = yields.map(y => ({ name: y.maturity, value: y.yield }));
        setYieldCurveData(formattedYields as any); // Cast needed for recharts compatibility if types differ
      } catch (err: any) {
        console.error(`Failed to fetch yield curve data:`, err);
        setYieldCurveError(`Could not load yield curve: ${err.message || 'Check API setup.'}`);
        setYieldCurveData([]);
      } finally {
  ...
  const getStatusBadgeVariant = (status: boolean | undefined): 'default' | 'destructive' | 'secondary' => {
   ...
   const getActionIcon = (type: AgentAction['type']) => {
        switch (type) {
            ...
            case 'REBALANCE': return <Scale className="h-4 w-4 text-blue-500" />; // Scale for rebalance
            ...
            case 'STRATEGY_UPDATE': return <Zap className="h-4 w-4 text-orange-500" />; // Zap for strategy update
            default: return <Landmark className="h-4 w-4" />; // Landmark as default
        }
   }
         ...
         <CardContent>
            <div className="h-[250px] w-full relative border rounded-md p-2">
                 {loadingYieldCurve ? (
                     ...
                     <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/80 z-10 rounded-md text-center p-4"> <AlertTriangle className="h-6 w-6 text-destructive mb-2" /> <p className="text-sm font-medium text-destructive">Yield Curve Error</p> <p className="text-xs text-destructive">{yieldCurveError}</p> </div>
                 ) : yieldCurveData.length === 0 ? (
                      <div className="absolute inset-0 flex items-center justify-center bg-background/80 z-10 rounded-md"> <p className="text-sm text-muted-foreground p-4 text-center">No yield curve data available.</p> </div>
                 ) : (
                     <ChartContainer config={chartConfig} className="h-full w-full">
                         {/* Using AreaChart for consistency, but LineChart might be better for yield curves */}
                         <AreaChart data={yieldCurveData} margin={{ top: 5, right: 10, left: -15, bottom: 5 }}>
                             <CartesianGrid strokeDasharray="3 3" vertical={false} />
                             {/* XAxis based on maturity name */}
        ...
        <CardContent>
           <ScrollArea className="h-[400px] w-full pr-4 border rounded-md">
            {loadingActions ? (
                 ...
                 <Alert variant="destructive" className="m-4"> <AlertTriangle className="h-4 w-4" /> <AlertTitle>Actions Error</AlertTitle> <AlertDescription>{actionsError}</AlertDescription> </Alert>
            ) : actions.length === 0 ? (
                <div className="flex items-center justify-center h-full"> <p className="text-center text-muted-foreground py-10">No agent actions recorded yet.</p> </div>
            ) : (
